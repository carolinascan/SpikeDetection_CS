...
save(['C:\Users\Carolina\Documents\GitHub\SpikeDetection_Carolina\Analisi segnali\HT ANALISI\HT\MULTI UNIT V2\timestamps_all_andtime.mat'],'timestamps_found_all','elapsed_time')
%% load locations
...
for i=1:length(SNR_param)
    for j=1:length(list_parameter)
        analysis_ht= signal_analysis_multi_unit(timestamps_found_all{j,i},loc_spike_s1,signal);
        eff(j,1)=analysis_ht.eff;
        TP(j,1)=analysis_ht.tp;
        FP(j,1)=analysis_ht.fp;
        TN(j,1)=analysis_ht.tn;
        FN(j,1)=analysis_ht.fn;
        Sensitivity(j,1)=analysis_ht.sensitivity;
        Specificity(j,1)=analysis_ht.specificity;
        FN_rate(j,1)=analysis_ht.FN_rate;
        FP_rate(j,1)=analysis_ht.FP_rate;
        FDR(j,1)=analysis_ht.false_discovery_rate;
        accuracy(j,1)=analysis_ht.accuracy;
        F1_score(j,1)=analysis_ht.F1_score;
        precision(j,1)=analysis_ht.precision;
        FOR(j,1)=analysis_ht.FOR;
        MCC(j,1)=analysis_ht.MCC;
        git{j,:}=analysis_ht.git;
        NPV(j,:)=analysis_ht.NPV;
        TS_TP{j,:}=analysis_ht.TS_tp;
        TS_TP_from_model{j,:}=analysis_ht.TS_tp_from_model;
        clear analysis
    end
    TP_all(i,:)=TP;
    FP_all(i,:)=FP;
...
% mkdir(['C:\Users\Carolina\Documents\GitHub\SpikeDetection_Carolina\Analisi segnali\HT ANALISI\HT\SINGLE UNIT V2\easy']);
save(['C:\Users\Carolina\Documents\GitHub\SpikeDetection_Carolina\Analisi segnali\HT ANALISI\HT\SINGLE UNIT V2\easy\timestamps_all_andtime.mat'],'timestamps_found_all','elapsed_time','list_parameter')
%% load locations
...
for i=1:length(SNR_param)
    for j=1:length(list_parameter)
        analysis_ht= signal_analysis_single_unit(timestamps_found_all{j,i},invgau_matrix,signal);
        eff(j,1)=analysis_ht.eff;
        TP(j,1)=analysis_ht.tp;
        FP(j,1)=analysis_ht.fp;
        TN(j,1)=analysis_ht.tn;
...
save(['C:\Users\Carolina\Documents\GitHub\SpikeDetection_Carolina\Analisi segnali\HT ANALISI\HT\SINGLE UNIT V2\easy\ANALYSIS.mat'],'TP_all','FP_all','EFF','list_parameter','list_snr','TN_all','FN_all','SPECIFICITY','SENSITIVITY','PRECISION','GIT_all','FOR_all','FN_rate_all','FP_rate_all','MCC_all','FDR_all','F1_SCORE','ACCURACY','NPV_all','TS_TP_all','TS_TP_FROM_MODEL_all')
%% PLOTS
high=list_snr(1);
mid=list_snr(5);
low=list_snr(10);
...
%% parametro 4
snr=10;
tpr=(SENSITIVITY(snr,4))
tnr=(SPECIFICITY(snr,4))
ppv=(PRECISION(snr,4))
fnr=(FN_rate_all(snr,4))
npv=(NPV_all(snr,4))
fpr=(FP_rate_all(snr,4))
fdr=(FDR_all(snr,4))
FoR=(FOR_all(snr,4))
csi=(EFF(snr,4))
acc=(ACCURACY(snr,4))
