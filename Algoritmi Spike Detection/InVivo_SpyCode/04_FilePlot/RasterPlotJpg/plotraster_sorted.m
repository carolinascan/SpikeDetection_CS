function [] = plotraster_sorted(start_folder, end_folder, fs, starttime, endtime, startend)
...
%     cd(peakfolderdir(f).name)
    ...
    for i = 1:numphasefiles
        ...
        electrode = str2double(phasefiles{i}(end-7:end-6));
        evector = [evector; electrode];
        if endtime<0
            endtimes=length(peak_train);
        else 
            endtimes=endtime;
        end

         realspiketimes = find(peak_train(starttime+1:endtimes));
                realspiketimes = (realspiketimes+starttime)/fs; % correct X-scale [sec]
                spiketms = [realspiketimes; endtimes/fs+10; endtimes/fs+11]; % To avoid 0 or 1 spikes - at least two
    ...
    if ~isempty(spkTs)
        spkTs_s = spkTs/fs;
        %bar(spiketms, sign(spiketms), 0.001);
