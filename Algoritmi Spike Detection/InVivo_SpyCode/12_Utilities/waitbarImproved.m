...
% 
% A Matlab waitbar compatible replacement that is faster and more
% functional. 
% 
% ARGUMENTS:
...
%              'DelayPeriod'   (default: 5.0)        Time until window is
%              shown, [0, 60]. 
%              'LingerPeriod'  (default: 0.0)        Time until window is hidden, [0, 60]. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	persistent waitBarWin; 
	
	
	%%% Check if the minimum time between updates is up.
	thistime= clock;
	if(~isempty(waitBarWin) && (abs(thistime(6)-waitBarWin.last) <= waitBarWin.mintime))
	...
	waitBarWin.last=		thistime(6);
	if(~isfield(waitBarWin, 'mintime'))
		waitBarWin.mintime=	0.5; % [seconds]			% Default minimum update period.
	end
	...
	if(nargin == 0)
		...
		try 
			if(isempty(waitBarWin.start) || isempty(findstr(db(2).name, 'timercb')))
				...
				waitBarWin.msg=			'';
				set(waitBarWin.fighdl, 'Name', waitBarWin.figtitle);	% Remove progress from window title.
				
				set(waitBarWin.fighdl, 'Visible', 'off');
				waitBarWin.start=			[];
				waitBarWin.starttime=		'';
				waitBarWin.vistimer=		handle_timer(waitBarWin.vistimer, -1, '');
				waitBarWin.lingertimer=	handle_timer(waitBarWin.lingertimer, -1, '');
			end
			progf=	waitBarWin.fighdl;
		catch
		end
		return;
	end
	...
	try	
		if(~waitBarWin.text)
			ud=		get(waitBarWin.fighdl, 'UserData');		% Does waitBarWin.fighdl exist?
			if(isempty(ud) || (ud ~= 6808795978))
				waitBarWin=	make_waitbarwin(waitBarWin); 
			end
		end
	...
	if(isempty(waitBarWin.starttime))
		waitBarWin.starttime=	sprintf('Started at %d:%02d:%02d, cpu time used: ', ...
									thistime(4), thistime(5), floor(thistime(6)));
		waitBarWin.startcpu=	cputime;
	end
	...
	if(isnumeric(prog) && (numel(prog) == 1))			%%%%% Update waitbar.
		...
		elseif((nargin >= 2) && ischar(varargin{2}) && (2*floor(nargin/2) == nargin))
			...
			if(~waitBarWin.text && (nargin > 2) && ~strcmp(varargin{3}, 'CreateCancelBtn'))
				waitBarWin=		set_properties(waitBarWin, prog, varargin{3:end});
			end
		elseif(nargin >= 2)
		...
		elseif(prog >= 1)
			...
			waitBarWin.lingertimer=	handle_timer(waitBarWin.lingertimer, waitBarWin.linger, [mfilename ';']);
			waitBarWin.start=		[];							% Mark that we're done.
		elseif((prog == 0) || isempty(waitBarWin.start))
			waitBarWin.start=	clock;							% (Re)start timing.
			waitBarWin.head=	true;							% We need to rewrite progress header.
			etastr=		'';								% No ETA at start.
		else
			s=		etime(clock,waitBarWin.start);				% Time used so far.
			etastr=	sprintf('%3d%% [%s]', floor(100*prog), get_timestr(1 + s*(1/prog - 1)));
		end
		
		%% Update the progress information. 
		if(waitBarWin.text)
			waitBarWin=				text_progress(prog, etastr, msgstr, waitBarWin);
		else
			set(waitBarWin.etahdl,  'String', sprintf('%s\n', etastr));
			set(waitBarWin.timehdl, 'String', [waitBarWin.starttime get_timestr(cputime - waitBarWin.startcpu)]);
			set(waitBarWin.fighdl,  'Name',   [num2str(floor(100*prog)) '% ' waitBarWin.figtitle]);
			set(waitBarWin.proghdl, 'XData',  [0 prog prog 0]);
			waitBarWin.winpos=			get(waitBarWin.fighdl, 'Position');% Save progress window position. 
			if(ischar(msgstr)), set(waitBarWin.msghdl, 'String', sprintf('%s\n', msgstr));	end
			drawnow;									% Force redraw.
		end
		if(ischar(msgstr)),		waitBarWin.msg= msgstr;	end
	...
	%%% Start delay timer. 
	if(strcmpi(get(waitBarWin.fighdl, 'Visible'), 'off') && isempty(waitBarWin.vistimer))
		waitBarWin.vistimer=	handle_timer(waitBarWin.vistimer, waitBarWin.delay, ...
				['try set(' num2str(waitBarWin.fighdl, 99) ', ''Visible'', ''on''); catch end']);
	end
	
	%%% Put the waitbar window on top of all others.
	children=		allchild(0);
	if(~waitBarWin.text && (numel(children) > 1) && (children(1) ~= waitBarWin.fighdl))
		uistack(waitBarWin.fighdl, 'top');
	end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	if(~isfield(data, 'text'))
		data.text=			false;
		data.maxlen=		0;
		data.delay=			0.2; % [seconds]			% Default delay until window is visible.
		data.linger=		0.2; % [seconds]			% Default delay until window is hidden.
...
%==========================================================================
%==========================================================================
function CloseRequestFcn(waitBarWinFighdl)
if ~strcmpi(get(waitBarWinFighdl,'waitstatus'),'waiting')
    delete(waitBarWinFighdl);
    return;
end
