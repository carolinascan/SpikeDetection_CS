...
%    progbar(pb,progress,msg)  update the progress and message on an existing progbar
%    progbar(pb,-1)            closes an existing progbar (no error if closed by user)
%    close(pb)                 alternative way to close (error if already closed)
%
...
if nargin<2
	...
	'userdata',info,...
	'name',info.title);
	set(handle,'position',[x-pw/2 y-th/2 pw th]);
	set(handle,'resize','off');
	
	% get color
	if ~isempty(which('rgb.m')) & 0
...
elseif nargin==2
	...
	if ~ishandle(pb) return; end % ignore if closed already
