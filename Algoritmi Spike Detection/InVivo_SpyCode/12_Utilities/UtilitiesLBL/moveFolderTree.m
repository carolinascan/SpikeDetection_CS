function [  ] = moveFolderTree(regExpToRet, regToFulfill, level, stringToBeReplaced, stingToReplace)

% Luca Leonardo Bologna, June 2008
%
% v 1.0 04/06/08

%MOVEFOLDERTREE given a starting folder, the script moves all the folders
%(and their content) to a given subfolder. The folders will be moved
%starting from the upper tree level passed through the "level" argument
% regExpToRet: regular expression used to retrieve the folder move
...
% e.g. call 1: moveFolderTree('_PeakDetectionMAT_Full_PLP2ms_RP1ms.*',{'45','48','46','49'}, 5, {'PeakDetectionMAT_Full_PLP'}, {'PeakDetectionMAT_PLP'})
% e.g. call 2: moveFolderTree('_PeakDetectionMAT_Splitted_PLP2ms_RP1ms.*',{'div'}, 4, {'PeakDetectionMAT_Splitted_PLP'}, {'PeakDetectionMAT_PLP'})
...
if isempty(names)
    display(['no folder fulfilling the given criteria. Exiting']);
    return
end
...
for i=1:length(names) %for every folder to move
    ...
    startName=charName(idxFileSep(max(1,length(idxFileSep)-level))+1:(idxPrev-2)); %extract the path containing up to the "upper" level the user want to move
    ...
    finalFolderComplete=fullfile(finalFolder,startName); %build the complete name of the folder to move
    if ~isdir(finalFolderComplete) %if it is not an existing directory, create it
        mkdir(finalFolderComplete);
    end
    movefile(char(names(i)),fullfile(finalFolderComplete,finalSubFolder),'f') %move the folder
