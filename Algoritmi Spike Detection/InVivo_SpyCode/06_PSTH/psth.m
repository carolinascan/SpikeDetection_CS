...
cancsample= round(cancw*fs/1000);          % canc window size [samples]
psthendsample=round(psthend*fs/1000);      % time length of the histogram [samples]
n= length(artifact);                % number of artifact
psth_count= zeros(round((psthend/bin)),1); % psthend/bin = total number of bin for the histogram
...
if (n>=1)                                                              % number of artifact must be at least 1
    for k=1:n                                                          % cycle over stimuli
        ...
        if (length(peak_train)-artifact(k)<psthendsample-1)  
            psthendsample= length(peak_train)-artifact(k)+1;
        ...
        bin_num=(round(peak_index/binsample));                          % bin with spikes        
