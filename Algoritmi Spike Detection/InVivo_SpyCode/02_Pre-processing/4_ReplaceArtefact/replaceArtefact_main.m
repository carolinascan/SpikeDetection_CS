% replaceArtefact_main.m 
% created by Valentina Pasquale, 05 May 2009
    ...
    while strcmpi(mulAnWinAnswer,'OK') && isempty(experimentFolders)
        ...
        if strcmpi(mulAnWinAnswer,'Cancel')
            ...
            if ~isempty(foldersList)
                ...
                % be performed
                experimentFolders=extractExperimentFoldersAnaFilesNames(foldersList);
                %if some folder has been found
...
%                         [mulAnWindHand,mulAnWindAnswer,chosenParameters]= multipleAnalysisWindow;
                        [replaceArtParam, flag] = replaceArtefact_getParam();
%                         %delete the window
...
%                             commonParameters=chosenParameters{1};
                            ...
                            for i=1:numFolders
                                ...
                                % output folder
%                                 [saveFolderName, successFlag] = createResultFolderNoUserQuestion(actualFolder, 'newArtefactDetection');
...
%                                     return
%                                 end
                                replaceArtefact_comput(actualFolder,replaceArtParam);
                            end
                            experimentFolders=[];
