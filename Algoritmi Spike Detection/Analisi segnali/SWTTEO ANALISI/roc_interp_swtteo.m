...
for i=1:length(list_snr)
    load(['C:\Users\Carolina\Documents\GitHub\SpikeDetection_Carolina\Analisi segnali\SWTTEO ANALISI\coordinate roc swtteo\XY_MU_snr' num2str(list_snr(i)) '.mat'])
    x=XY(:,1);
    y=XY(:,2);
    xf=flip(x);
    yf=flip(y);
...
%     save(['C:\Users\Carolina\Documents\GitHub\SpikeDetection_Carolina\Analisi segnali\ ANALISI\AUC HT\AUC_MU_SNR' num2str(list_snr(i)) '.mat'],'AUC')
    clear XY x y x_new x_interp y_new y_interp curr_x_unique
end

%% EXPO
for i=1:length(list_snr)
    load(['C:\Users\Carolina\Documents\GitHub\SpikeDetection_Carolina\Analisi segnali\SWTTEO ANALISI\coordinate roc swtteo\XY_expo_snr' num2str(list_snr(i)) '.mat'])
    x=XY(:,1);
    y=XY(:,2);
    x_new=unique(x);
    for curr_x_unique=1:length(x_new)
