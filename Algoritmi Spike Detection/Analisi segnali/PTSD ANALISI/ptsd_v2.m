...
save(['C:\Users\Carolina\Documents\GitHub\SpikeDetection_Carolina\Analisi segnali\PTSD ANALISI\PTSD\MULTI UNIT V2\PARAMETRO 1-20\timestamps_firstHalf_all_andtime.mat'],'timestamps_found_all','elapsed_time', '-v7.3')
...
save(['C:\Users\Carolina\Documents\GitHub\SpikeDetection_Carolina\Analisi segnali\PTSD ANALISI\PTSD\MULTI UNIT V2\PARAMETRO 1-20\timestamps_secondoHalf_all_andtime.mat'],'timestamps_found_all','elapsed_time', '-v7.3')
...
save(['C:\Users\Carolina\Documents\GitHub\SpikeDetection_Carolina\Analisi segnali\PTSD ANALISI\PTSD\MULTI UNIT V2\PARAMETRO 1-20\ANALYSIS_firstHalf.mat'],'TP_all','FP_all','EFF','list_parameter','list_snr','TN_all','FN_all','SPECIFICITY','SENSITIVITY','PRECISION','GIT_all','FOR_all','FN_rate_all','FP_rate_all','MCC_all','FDR_all','F1_SCORE','ACCURACY','NPV_all','TS_TP_all','TS_TP_FROM_MODEL_all')
%% SECOND HALF
load('C:\Users\Carolina\Documents\GitHub\SpikeDetection_Carolina\Dati\SIGNAL V2\snr_0.16498.mat')
for i=1:length(list_snr)/2
    for j=1:length(list_parameter)
        for k=1:length(list_PLP)
            ts=m.timestamps_found_all(j,k,i);
            TS=ts{:};
            analysis_ptsd= PTSD_signal_analysis_multi_unit(TS,loc_spike_s1,signal);
...
for l=1:length(list_snr)
    if l<=5
        ...
        makeFig_MAXIMIZE(ax,list_PLP,(squeeze(EFF(l,:,:))),list_parameter,list_snr(l),title,method)
    elseif l>=6
        idx2=1:10;
        ax2=subplot(5,2,idx2(l))
        makeFig_MAXIMIZE(ax2,list_PLP,(squeeze(EFF2(l-5,:,:))),list_parameter,list_snr(l),title,method)    
    end
end 
